...
func runExternalClusterTest(t *testing.T, isAdmin bool) {
	...
	rbdPoolsForShareStr, ok := caseConfig["rbdPoolsForShare"]
	if !ok {
		t.Fatal("Could not obtain test case rbdPoolsForShare configuration")
	}
	if rbdPoolsForShareStr != "" {
		rbdPoolsForShare = strings.Split(rbdPoolsForShareStr, ",")
	}
	cephfsPoolsForShareStr, ok := caseConfig["cephfsPoolsForShare"]
	if !ok {
		t.Fatal("Could not obtain test case cephfsPoolsForShare configuration")
	}
