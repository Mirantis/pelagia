...
* Pelagia now has Rook as a separate chart allowing the user to disable the Pelagia-based Rook setup and use a different Rook chart.
...
3. Switch to the [Gateway API](https://gateway-api.sigs.k8s.io/guides/getting-started/introduction/) due to
   the deprecation of the Ingress NGINX controller. The `Gateway` object and Controller must be configured separately.
     ...
     Once the Gateway API HTTPRoute is enrolled, disable Ingress in Helm values `lcmConfig.useIngress: false` and completely
     remove the `ingressConfig` section from `CephDeployment` spec if present.
