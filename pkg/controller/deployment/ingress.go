...
func (c *cephDeploymentConfig) ensureIngressProxy() (bool, error) {
	...
	for _, rgw := range c.cdConfig.cephDpl.Spec.ObjectStorage.Rgws {
		...
		if ingressConfig.ControllerClassName == defaultIngressClass {
			ingressConfig.Annotations["nginx.ingress.kubernetes.io/upstream-vhost"] = buildHostName(rgw.Name, ingressConfig.TLSConfig.Hostname, ingressConfig.TLSConfig.Domain)
		}
		...
		if ingressConfig.TLSConfig.TLSSecretRefName != "" {
			...
			if ingressConfig.TLSConfig.Hostname != "" {
				...
				tlsSecretName = buildRGWName(rgw.Name, fmt.Sprintf("tls-public-%v", lcmcommon.GetCurrentUnixTimeString()))
			}
		...
		tlsSecretToDelete := ""
		if ingress != nil {
			hostName := buildHostName(rgw.Name, ingressConfig.TLSConfig.Hostname, ingressConfig.TLSConfig.Domain)
