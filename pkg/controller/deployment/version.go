...
func (c *cephDeploymentConfig) ensureCephClusterVersion() error {
	...
	if cephCluster.Spec.CephVersion.Image != c.cdConfig.currentCephImage {
		...
		if c.cdConfig.aes256kUpgrade {
			...
			// pin AES type for CSI keys. Operator should manually rotate them to aes256k after upgrade
			cephCluster.Spec.Security.CephX.CSI.KeyType = cephv1.CephxKeyTypeAes
			// ensure that current used CSI key will remain active during future rotation
			cephCluster.Spec.Security.CephX.CSI.KeepPriorKeyCountMax = 1
