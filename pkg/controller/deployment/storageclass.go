...
func (c *cephDeploymentConfig) ensureStorageClasses() (bool, error) {
	...
	for _, cephDplPool := range c.cdConfig.cephDpl.Spec.Pools {
		...
		storageResource := generateStorageClassPoolBased(c.lcmConfig.RookNamespace, cephDplPool, c.lcmConfig.RookNamespace, c.cdConfig.cephDpl.Spec.Cluster.External.Enable)
		found := false
		for _, storageClass := range storageClassesList.Items {
	...
	err = c.createStorageClasses(storageClassesToCreate, c.cdConfig.cephDpl.Spec.Cluster.External.Enable)
	if err != nil {
