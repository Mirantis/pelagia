...
NewPortLoop:
	...
	updateRequired = updateRequired || lcmcommon.AlignBaseLabels(*c.log, "service", &externalSvc.ObjectMeta, externalSvcResource.Labels)
	if updateRequired {
...
func (c *cephDeploymentConfig) ensureRgwCaBundleCert(rgwName, certRef, caBundleRef string, servedByIngress, usedByRockoon bool) (bool, error) {
	...
	c.log.Debug().Msgf("rgw '%s' has no provided cabundle secret ref, using '%s/%s' to store cabundle", rgwName, c.lcmConfig.RookNamespace, defaultCaBundleCertName)
	return changed, nil
}
